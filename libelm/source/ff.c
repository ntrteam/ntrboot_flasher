...
FRESULT f_mkfs (
	...
	DWORD priv_sectors	/* Sectors that are hidden */
)
{
...
//TODO: mod b_vol to be 2 GB for n3ds and 1GB for old 3ds
	...
	if (fmt == FS_FAT32) {
		...
		mem_cpy(tbl + BS_VolLab32, "NO NAME2   " "FAT32   ", 19);	/* Volume label, FAT signature */
	} else {
		ST_DWORD(tbl + BS_VolID, n);		/* VSN */
		ST_WORD(tbl + BPB_FATSz16, n_fat);	/* Number of sectors per FAT */
		tbl[BS_DrvNum] = 0x80;				/* Drive number */
		tbl[BS_BootSig] = 0x29;				/* Extended boot signature */
		mem_cpy(tbl + BS_VolLab, "NO NAME2   " "FAT     ", 19);	/* Volume label, FAT signature */
	}
